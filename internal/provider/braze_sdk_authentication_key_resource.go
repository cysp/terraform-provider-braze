...
func (r *brazeSDKAuthenticationKeyResource) Create(
	...
	data, err := r.providerData.sdkAuthenticationKeys.Create(ctx, plan)
	if data.ID.ValueString() != "" {
		resp.Diagnostics.Append(setSDKAuthenticationKeyIdentityAndState(
			ctx, resp.Identity, &resp.State, data.AppID.ValueString(), data.ID.ValueString(), &data,
		)...)
	}
	...
	if err != nil {
		...
		if data.ID.ValueString() != "" {
			detail = "Braze created SDK Authentication key " + data.ID.ValueString() + "; its identity is preserved in Terraform state. " + detail +
				" Resolve the verification failure and confirm the remote key before adopting it again. A failed create may taint the key; a currently primary key must be re-imported or deliberately untainted instead of replaced."
		}

		resp.Diagnostics.AddError("Failed to create SDK Authentication Key", detail)
	}
}
